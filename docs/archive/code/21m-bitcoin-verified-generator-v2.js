...
const BitcoinContentTracker = require('./lib/bitcoin-content-tracker.js');
...
const contentTracker = new BitcoinContentTracker();
...
/**
 * Log content selection for debugging
 */
function logSelection(topic, lastUsed = null) {
  const dateStr = new Date().toISOString().split('T')[0].replace(/-/g, '');
  const logFile = path.join(LOG_DIR, `21m-bitcoin-selections-${dateStr}.log`);
  
  const entry = `[${new Date().toISOString()}] Selected: ${topic} (last_used: ${lastUsed || 'never'})\n`;
  ...
  try {
    ...
    console.log(`  📝 Logged selection: ${topic}`);
  } catch (err) {
 ...
 * Filters out recently used content (14-day cooldown)
 */
function getBestBitcoinContentFromDatabase() {
  ...
  console.log(`   Content tracked: ${stats.total_tracked}`);
  console.log(`   Recently used: ${stats.recently_used}`);
  ...
  try {
    ...
    let bitcoinContent = drafts.filter(d =>
      d.category && (
    ...
    // Filter out recently used content using tracker
    const availableContent = contentTracker.filterUnused(bitcoinContent, 14);
    
    if (availableContent.length === 0) {
      console.log('  ⚠️ All recent content used, checking 30-day filter...');
      // Fallback to 30 days
      const extendedContent = contentTracker.filterUnused(bitcoinContent, 30);
      if (extendedContent.length === 0) {
        console.log('  ⚠️ All content used in 30 days, using full list...\n');
        bitcoinContent = bitcoinContent;
      } else {
        bitcoinContent = extendedContent;
      }
    } else {
      bitcoinContent = availableContent;
    }
    ...
    if (sorted.length === 0) {
      console.log('  ℹ️  No high-quality Bitcoin content (score >= 7) available\n');
      return null;
    }

    const best = sorted[0];
    console.log(`  ✓ Found ${sorted.length} high-quality entries (${contentTracker.filterUnused(sorted, 14).length} unused in 14 days)`);
...
async function generateContent() {
  ...
  try {
    ...
    // Mark content as used in tracker (14-day cooldown)
    if (dbContent && dbContent.id && !DRY_RUN) {
      const lastUsed = contentTracker.wasRecentlyUsed(dbContent.id) 
        ? contentTracker.data.topics[dbContent.id.toString()]
        : null;
      contentTracker.markUsed(dbContent.id);
      logSelection(dbContent.topic, lastUsed);
    }
