...
- **NOTE:** Dashboard reads from Clawdbot via `clawdbot cron list --json`

---

## Additional System Jobs (Clawdbot)

These jobs run via Clawdbot cron (not system crontab):

| Job | Purpose |
|-----|---------|
| **Reminder Checker** | Checks every 5 min for due reminders, posts to Slack |
| **Performance Check** | Runs every 6 hours, monitors system health |
| **Weekly Summary** | Generates weekly automation report every Sunday 8 PM |
