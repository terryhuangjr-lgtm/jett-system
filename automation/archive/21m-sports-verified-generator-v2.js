...
async function generateContent() {
  ...
  try {
    ...
    // Check if athlete was recently used (14-day cooldown)
    const playerName = finding.player;
    if (athleteTracker.wasRecentlyUsed(playerName, 14)) {
      const lastUsed = athleteTracker.data.athletes[playerName.toLowerCase()];
      console.error(`\n⚠️  ${playerName} was used within last 14 days (last: ${lastUsed})`);
      console.error('   Run 21m-sports-real-research.js to get fresh athletes');
      return 1;
    }
    ...
    // Mark athlete as used in tracker
    const lastUsed = athleteTracker.data.athletes[playerName.toLowerCase()] || null;
    athleteTracker.markUsed(playerName);
    logSelection(playerName, lastUsed);
