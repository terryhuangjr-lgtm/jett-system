#!/usr/bin/env node
/**
 * Dirk Nowitzki Refractors 1998-2005 Search
 */
...
async function searchDirkRefractors() {
  ...
  try {
    const search = {
      name: 'Dirk Nowitzki Refractors 1998-2005',
    ...
    const filtered = items.filter(item => {
      ...
      // Must mention refractor or chrome/prizm (refractor-like)
      if (!title.includes('refractor') && !title.includes('chrome') && 
          !title.includes('prizm') && !title.includes('atomic') &&
          !title.includes('precious metal')) {
        return false;
      }
      
      // Year range 1998-2005
      const yearMatch = title.match(/199[8-9]|200[0-5]|1998|1999|2000|2001|2002|2003|2004|2005/);
    ...
    const reportFile = path.join(__dirname, 'results', `dirk-refractors-${new Date().toISOString().split('T')[0]}.json`);
...
function printResults(top10, duration, reportFile, summary) {
  console.log('═══════════════════════════════════════════════════');
  console.log('🏀 TOP 10 DIRK NOWITZKI REFRACTORS (1998-2005)');
  console.log('═══════════════════════════════════════════════════\n');
  ...
  console.log(`   Refractor singles: ${summary.afterFiltering}`);
...
module.exports = searchDirkRefractors;
