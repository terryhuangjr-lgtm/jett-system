#!/usr/bin/env node
/**
 * Shaquille O'Neal Rare Numbered Cards /25 or Less
 */
...
async function searchShaqRare() {
  ...
  try {
    const search = {
      name: "Shaquille O'Neal Numbered /25 or Less",
    ...
    // Filter for numbered /25 or less
    const filtered = items.filter(item => {
      ...
      // Must be numbered /25 or less
      // Look for patterns: /25, /10, /5, /1, 1/1, etc.
      const serialPatterns = [
        /\/\s?([1-9])\b/,        // /1 through /9
        /\/\s?(1[0-9])\b/,       // /10 through /19  
        /\/\s?(2[0-5])\b/,       // /20 through /25
        /\b1\s?\/\s?1\b/         // 1/1
      ];
    ...
    const reportFile = path.join(__dirname, 'results', `shaq-rare-${new Date().toISOString().split('T')[0]}.json`);
...
function printResults(top10, duration, reportFile, summary) {
  console.log('═══════════════════════════════════════════════════');
  console.log('🏀 TOP 10 SHAQ RARE NUMBERED CARDS (/25 OR LESS)');
  console.log('═══════════════════════════════════════════════════\n');
  ...
  console.log(`   Numbered /25 or less: ${summary.afterFiltering}`);
